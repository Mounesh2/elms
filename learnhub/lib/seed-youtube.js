...
function logToFile(msg) {
  if (msg instanceof Error) {
    fs.appendFileSync('seed_debug.txt', `ERROR: ${msg.message}\nSTACK: ${msg.stack}\n`)
  } else {
    fs.appendFileSync('seed_debug.txt', (typeof msg === 'object' ? JSON.stringify(msg, null, 2) : msg) + '\n')
  }
}
...
async function main() {
  ...
  const techCreators = [
    ...
    {
       name: 'Piyush Garg',
       email: 'piyush@piyushgarg.dev',
       headline: 'Full Stack & Web Architect',
       bio: 'Teaching modern web stacks and architecture.',
       avatar: 'https://yt3.googleusercontent.com/ytc/AIdro_nNfV-b7v-Lp-V8W-L_gVl-K-V-L-K-V=s176-c-k-c0x00ffffff-no-rj',
       courses: [
         {
           title: 'Complete Next.js 14 Course',
           subtitle: 'Master Next.js App Router and Server Components.',
           description: 'Learn Next.js 14 from basic to production deployment.',
           slug: 'piyush-nextjs-14',
           category: 'web-development',
           thumbnail: 'https://i.ytimg.com/vi/uQeIDHSp6_k/maxresdefault.jpg',
           price: 0,
           level: 'Intermediate'
         }
       ]
    },
    {
       ...
       headline: 'CS Education Specialist',
       bio: 'Sanchit Jain simplifies CS subjects for competitive exams.',
       avatar: 'https://yt3.googleusercontent.com/ytc/AIdro_nNfV-b7v-Lp-V8W-L_gVl-K-V-L-K-V=s176-c-k-c0x00ffffff-no-rj',
       courses: [
         {
           title: 'DBMS Complete Playlist',
           subtitle: 'Database Management Systems for Engineering.',
           description: 'SQL, Normalization, Transactions, and more explained simply.',
           slug: 'kg-dbms',
           category: 'computer-science',
           thumbnail: 'https://i.ytimg.com/vi/3Zp7P0yM3B0/maxresdefault.jpg',
           price: 0,
           level: 'Beginner'
         }
       ]
  ...
  for (const creator of techCreators) {
    ...
    try {
      ...
      console.log(`  Profile upserted for: ${profile.fullName}`)
      ...
      console.error(`Error processing ${creator.name}:`)
      logToFile(`Error processing ${creator.name}:`)
      logToFile(err)
      if (err.code) logToFile(`Error Code: ${err.code}`)
      if (err.meta) logToFile(`Error Meta: ${JSON.stringify(err.meta)}`)
    }
  }
